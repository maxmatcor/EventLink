...
public class helpControler extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String action = request.getParameter("action"); //INICIALIZA UN PARAMETRO CON EL VALOR
		String idstr = request.getParameter("id"); //INICIALIZA UN PARAMETRO 
		
		
		if ("delete".equals(action)) {	//SI LA ACCION ES DELETE
			if (idstr != null) { //Y EL PARAMETRO ID NO ES NULO 
				int id = Integer.parseInt(idstr);
				Object result =help.DeleteHelp(id);
				
				ArrayList<helpData> helpdatas = help.FindAll();// DAO 
				request.setAttribute("helpdatas", helpdatas); //PIDE EL PAQUETE
				
				if(result==null){
				sesion.setAttribute("msg", "1"); //MARCADO COMO LEIDO
				}else{sesion.setAttribute("msg", "0");			//HUBO UN ERROR VOLVE A INTENTARLO	
				}		
